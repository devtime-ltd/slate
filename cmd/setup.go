...
func runSetup(cmd *cobra.Command, args []string) error {
	...
	if err := runProxyTrust(cmd, args); err != nil {
		fmt.Printf("  ⚠️  CA trust failed: %v\n", err)
		fmt.Println("  Run `slate proxy trust` manually if needed.")
	}

	fmt.Println("\n✅ Setup complete. Run `slate init <scaffold>` in a project to get started.")
	return nil
}
...
func runTeardown(cmd *cobra.Command, args []string) error {
	...
	fmt.Println("  ✅ Workspace caches are removed with `slate rm`")

	fmt.Println("\n✅ Teardown complete. Workspaces and project files are untouched.")
	return nil
}
