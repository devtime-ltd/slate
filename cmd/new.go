...
func createWorkspace(name, branch string, bg, cd, adopt, bare bool) error {
	...
	// Written now, cleared after the first agent session: without it, a
	// workspace provisioned without a TTY (no up hook fires) would present
	// its first `slate agent` entry as a continuation.
	if err := os.WriteFile(agentPendingMarker(wsDir), nil, 0o644); err != nil {
		fmt.Printf("  warning: could not mark the agent first run as pending: %v\n", err)
	}
