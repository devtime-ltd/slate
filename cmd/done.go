...
func prForBranch(dir, branch, def, tip string) (prRef, bool, error) {
...
func checkLanded(mainRoot, wsDir string) landedEvidence {
	...
	summary, dirty, derr := worktreeStatus(wsDir)
	if derr != nil {
	...
	for _, ref := range []string{"refs/remotes/origin/" + def, "refs/heads/" + def} {
		if _, err := gitIn(wsDir, "rev-parse", "--verify", "--quiet", ref); err != nil {
			continue
		}
		if isAncestorIn(wsDir, tip, ref) {
	...
	if merged {
		...
		ev.ok = true
		defTip, _ := gitIn(wsDir, "rev-parse", mergedInto)
		switch {
		case tip == defTip:
		...
		default:
			...
			if pr, found, err := prForBranch(mainRoot, branch, def, tip); err == nil && found && pr.State == "MERGED" && pr.BaseRefName == def && pr.HeadRefOid == tip {
				...
				ev.evidence = "nothing beyond " + mergedInto + " · worktree clean"
			}
		}
		return ev
	}
