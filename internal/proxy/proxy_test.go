...
type stubCaddy struct {
	...
	servers   map[string][]string // foreign servers: name -> listen addresses
	tlsBody   string              // last body PUT to /config/apps/tls
}
...
func (s *stubCaddy) start(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet {
			...
			if s.rejectPUT && r.Method == http.MethodPut && r.URL.Path == "/config/apps/http/servers/slate" {
		...
		case "/config/apps/http/servers":
			...
			for name, listen := range s.servers {
				out[name] = map[string][]string{"listen": listen}
			}
			if s.listen != nil {
				out["slate"] = map[string][]string{"listen": s.listen}
			}
...
// On a foreign Caddy, slate adds only its server and never touches the host's
// TLS configuration - issuing certs for slate's .test hosts is that Caddy's job.
func TestEnsureServerLeavesForeignTLSUntouched(t *testing.T) {
	...
	if !stub.called("PUT", "/config/apps/http/servers/slate") {
		t.Errorf("should add the server node; calls = %v", stub.calls)
	}
	if stub.called("PUT", "/config/apps/tls") || stub.called("PUT", "/config/apps/tls/automation/policies") {
		t.Errorf("must not write the foreign Caddy's TLS config; calls = %v", stub.calls)
	}
}
...
// P1: on a foreign Caddy already serving :80/:443, slate cannot add its own
// server (Caddy requires unique listeners) and must refuse up front with
// guidance rather than a mid-provision failure.
func TestEnsureServerRefusesWhenForeignServerOwnsListener(t *testing.T) {
	stub := &stubCaddy{
		listen:  nil,
		hasTLS:  true,
		servers: map[string][]string{"srv0": {"0.0.0.0:443", "0.0.0.0:80"}},
	}
	stub.start(t)
	withOwnership(t, false)
	...
	err := EnsureServer(true)
	if err == nil {
		t.Fatal("EnsureServer() should refuse when a foreign server owns the listeners")
	}
	if stub.called("PUT", "/config/apps/http/servers/slate") {
		t.Errorf("must refuse before attempting to add the server; calls = %v", stub.calls)
	}
	if stub.called("POST", "/load") {
		t.Errorf("must never fall back to /load; calls = %v", stub.calls)
	}
}
